    ...
    estados: [
        {
            ...
            regiao: 'Sudeste',
            cidades: [
                { "nome": "São Paulo", "id": "3829" },
                { "nome": "Adamantina", "id": "3267" },
                { "nome": "Adolfo", "id": "3268" },
                // Outras cidades...
            ]
        },
...
// Função para obter estados por região (comparação sem distinguir maiúsculas de minúsculas)
function getEstadosRegiao(regiao) {
    const estadosRegiao = [];
    const regiaoLowerCase = regiao.toLowerCase(); // Converter para minúsculas
    for (let i = 0; i < estadosCidades.estados.length; i++) {
        const estado = estadosCidades.estados[i];
        if (estado.regiao.toLowerCase() === regiaoLowerCase) { // Comparação sem distinguir maiúsculas de minúsculas
            estadosRegiao.push({
                uf: estado.sigla,
                nome: estado.nome
            });
...
document.getElementById('searchForm').addEventListener('submit', function (event) {
    ...
    const estados = getEstadosRegiao(input);
    const regionStatesDiv = document.getElementById('regionStates');
    regionStatesDiv.innerHTML = ""; // Limpa o conteúdo anterior
    if (estados.length > 0) {
        estados.forEach(function (estado) {
            const estadoInfo = document.createElement('p');
            estadoInfo.textContent = `${estado.uf} - ${estado.nome}`;
            regionStatesDiv.appendChild(estadoInfo);
        });
    } else {
        regionStatesDiv.innerHTML = "<p>No states found in this region.</p>";
    }
});