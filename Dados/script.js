...
// Função para obter dados do estado por sigla
...
document.getElementById('searchForm').addEventListener('submit', function (event) {
    ...
    const estado = getDadosEstado(input);
    if (estado) {
        document.getElementById('stateInfo').innerHTML = `
            <h2>${estado.uf} - ${estado.nome}</h2>
            <p><strong>Capital:</strong> ${estado.capital}</p>
            <p><strong>Região:</strong> ${estado.regiao}</p>
        `;
    } else {
        document.getElementById('stateInfo').innerHTML = "<p>Estado não encontrado.</p>";
    }
});